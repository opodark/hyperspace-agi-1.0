...
# Endpoint protetti — richiedono secret se CLUSTER_SECRET è impostato
PROTECTED_PREFIXES = [
    '/gossip/',
    '/dreams/',
    '/peers/announce',
...
# Endpoint sempre pubblici (health, catalog, chat utente)
PUBLIC_PREFIXES = [
    '/health',
    '/docs',
    '/openapi',
    ...
    '/memory',
    '/auto-pull',
    '/peers',      # lettura peer ok
    '/resolve',
]


def _is_protected(path: str) -> bool:
    """Restituisce True se il path richiede autenticazione."""
...
class ClusterSecretMiddleware(BaseHTTPMiddleware):
    """
    Se HYPERSPACE_CLUSTER_SECRET è impostato, verifica che le chiamate
    verso gli endpoint P2P interni portino l'header corretto.
    Endpoint pubblici (health, chat, catalog) sono sempre accessibili.
    """
    ...
    async def dispatch(self, request: Request, call_next):
        if CLUSTER_SECRET and _is_protected(request.url.path):
            ...
            if token != CLUSTER_SECRET:
                ...
                return JSONResponse(
                    status_code=403,
                    content={'error': 'Invalid cluster secret', 'hint': f'Header {SECRET_HEADER} mancante o errato'}
                )
